// Script para verificar productos y generar órdenes físicas
...
const verifyAndGenerateOrders = async () => {
    try {
        console.log('✅ MongoDB de producción conectado');
        console.log('🔍 Verificando productos y generando órdenes físicas...\n');
        ...
        allProducts.slice(0, 5).forEach((product, index) => {
            console.log(`${index + 1}. ${product.name} - Activo: ${product.active} - Precio: $${product.price}`);
        });

        // Activar todos los productos si es necesario
        if (activeProducts.length === 0) {
            console.log('\n🔄 Activando todos los productos...');
            const updateResult = await Product.updateMany({}, { active: true });
            console.log(`✅ ${updateResult.modifiedCount} productos activados`);
        }
        ...
        const orderCount = 15; // Generar 15 órdenes físicas
