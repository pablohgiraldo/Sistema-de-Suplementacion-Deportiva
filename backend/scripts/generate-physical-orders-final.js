// Script para generar órdenes físicas usando la consulta correcta
...
const generatePhysicalOrdersCorrect = async () => {
    try {
        console.log('✅ MongoDB de producción conectado');
        console.log('🏪 Generando órdenes físicas con consulta correcta...\n');

        // Conectar a MongoDB
        await mongoose.connect(process.env.MONGODB_URI);
        console.log('✅ Conectado a MongoDB de producción');

        // Obtener usuarios y productos usando la consulta correcta
        const users = await User.find({}).limit(5);
        const products = await Product.find({ active: { $ne: false } }).limit(10);
        ...
        console.log(`✅ ${products.length} productos disponibles encontrados`);

        // Generar órdenes físicas
        const physicalOrders = [];
        const orderCount = 25; // Generar 25 órdenes físicas
        ...
        for (let i = 0; i < orderCount; i++) {
            ...
            const physicalOrder = {
                ...
                physicalSale: {
                    ...
                    cashierId: randomUser._id, // Usar ObjectId válido
